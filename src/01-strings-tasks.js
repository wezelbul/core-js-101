...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (chrCode >= upperStart && chrCode <= upperEnd) {
      startCharCode = upperStart;
    } else if (chrCode >= lowerStart && chrCode <= lowerEnd) {
    ...
    result += String.fromCharCode(startCharCode + addendum);
  }
  return result;
}
...
function getCardId(value) {
  ...
  switch (nominal) {
    ...
    case 'K':
      ...
      break;
    default:
      throw new Error('Out of card range');
  }
  switch (suit) {
    case '♣':
      suitFactor = 0;
  ...
  return nominalNum * suitFactor + nominalFactor;
}
